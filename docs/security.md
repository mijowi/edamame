...
- **Update check** (`src/app/update_check.rs`): a hardcoded `https://api.github.com` URL (not influenceable by file/config), 10 MB body cap, 10 s timeout, detached worker; the response is parsed only to extract a `tag_name` string. Nothing is downloaded or executed.
