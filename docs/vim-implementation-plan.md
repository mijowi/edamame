...
**Hint rows:** vim reuses edamame's own contextual + baseline hint row in every sub-mode — `hint_line_for` takes no vim argument. The modal keys (`i`/`v`/`:`/`/`, the Visual operators, `Esc` back to Normal, …) are vim-internal and the status bar already advertises the active sub-mode, so the hint line is spent on edamame-specific chords (checkbox toggle, link, table, Cut/Copy/Paste on a selection) rather than re-advertising vim's keys. A Visual selection lands on the shared selection row because vim Visual sets the editor `selection` just like a mouse drag.
...
> - **Hint rows: vim reuses edamame's own hint row in every sub-mode.** `hint_line_for` takes no vim argument — `flash.rs::hint_content` calls it with just the editor state, so vim Normal/Insert/Visual all show the same contextual + baseline chords the default handler shows (checkbox toggle, link, table, and Cut/Copy/Paste on a Visual selection — vim Visual sets the editor `selection` just like a mouse drag). The modal keys (`i`/`v`/`:`/`/`, the Visual operators, …) are vim-internal and the status bar badge already advertises the active sub-mode, so the row is spent on edamame-specific chords rather than re-advertising vim's keys. **An active search still wins** (its arm leads `hint_line_for`), so the search-flow chords + match counter stay visible while navigating matches.
