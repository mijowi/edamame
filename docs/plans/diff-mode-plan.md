...
- **Decision divider** — a synthetic line (no backing rope line) emitted at the old/new boundary of every hunk via `DiffLineSource::Decision`, so it sits below a delete-only hunk and above an insert-only one. Its text is focus-aware (`decision_divider_spans()` in `src/ui/diff_view.rs`): a **non-focused** divider is the bare checkbox plus a resolved label (`[ ]` while Pending, `[Y] Accepted`, `[N] Rejected`); the **focused** divider prepends a `>` caret and, while Pending, spells the keys inline — `> [ ] Reject [n] Accept [y]` — with the `[y]` / `[n]` glyphs pulled from the shared `diff_keys` table via `crate::input::diff_hint` so the prompt can never name a key the handler doesn't honor. **Reject leads and Accept follows**, so reading order mirrors the old-above-new stacking; unlike a directional glyph, order asserts nothing that goes false on an insert-only or delete-only hunk. Each label is a chip whose background is the *same* `diff_add_line` / `diff_delete_line` wash the add / delete rows carry, taking only the wash's `bg` and modifiers with the foreground pinned from `theme.normal` (`Color::Reset` when `normal` has none) — so the label, its key, and the block it acts on are one color, no new theme field can drift from the wash it names, and a theme that sets an `fg` on the wash against convention still can't produce an unreadable chip. In a monochrome theme both washes are a bare `REVERSED` and `normal.fg` is `None`, so the chips are indistinguishable and the mapping falls back to the prompt order and the `- ` / `+ ` side markers. Only the divider is color-coded; the diff hint row in `ui::bottom_region` stays uniform (`HintChord` carries no per-chord style). The base label glyphs (`[ ]` / `[Y]` / `[N]` / "Accepted" / "Rejected") still come from `decision_line_text()` in `src/diff/layout.rs`. Every divider also ends with a `(i/n)` position counter (1-based hunk index in document order over the total hunk count) — a separate span dimmed with `Modifier::DIM` (and the inherited bold cleared) so the index reads as quiet metadata; `DIM` rather than a muted color keeps it recessive in monochrome themes too. The counter is a *position* index, distinct from the status bar's `resolved/total` *progress* chip. The divider is a single-row status strip: it renders with `wrap = false` and is pinned to one row in the layout row cache, so the longer focused prompt never perturbs scroll math when focus moves. The background is a **neutral chrome surface**, not a `secondary` tint, so the colored foregrounds keep full contrast: the **focused** divider uses the per-state `theme.diff_decision_pending` / `diff_decision_accepted` / `diff_decision_rejected` on the heavier `surface_elevated` (set as the line's base style so the trailing-cell fill paints the whole row) plus an added bold — the focused-Pending fg is `secondary` so the inline prompt pops, the resolved states keep their green/red hue. A **non-focused** divider sits on the lighter `surface` so it recedes a step: `theme.diff_decision_unfocused` (muted fg, no bold) while Pending, and for `Accepted` / `Rejected` `build_line` keeps that background but swaps in the per-state green/red hue and adds `DIM`, so a resolved unfocused divider still signals its decision by color while staying quieter than the focused one. The hue is derived from the focused style (not the palette), so monochrome themes — where the focused style carries no color — keep a plain `DIM` strip and let the label text convey the state.
- **Stacked order** — old (delete) lines first, then the decision divider, then new (add) lines.
- **Focused hunk** — there is *no* gutter glyph or focus bar. Focus is shown by background intensity alone: the focused hunk's add/delete lines use `theme.diff_add_line` / `diff_delete_line` (stronger fill, itself darkened toward `bg` — see §9), while every non-focused hunk uses `theme.diff_add_line_unfocused` / `diff_delete_line_unfocused` (a fainter wash that recedes). The focused hunk's decision divider is additionally bolded and carries the `>` caret + inline prompt (see the Decision divider entry above). Every body line carries a two-cell `layout::line_marker` side gutter (`- ` delete, `+ ` add, two spaces on context, so the body columns line up), but that gutter names the *side*, not the focus: the focused/unfocused background split is the focus indicator for the add/delete lines, and the caret marks the focused divider even when those lines have scrolled out of view. The marker is prepended as its own span rather than folded into `line_text` (a hunk's inline highlight ranges index into the raw line's chars), and it is what keeps the side legible where color cannot carry it — a monochrome theme, and a delete-only or insert-only hunk where the divider has nothing to sit between. Anything that *measures* a diff line must include the marker and measure through `visual_rows_for_line`, not `visual_rows_of_str`: `render_line` reads `- ` / `+ ` / a two-space prefix as a hanging-indent marker, so a flat measurement would desync the layout row cache from the painter on every wrapped line.
...
> **These two are reused at render time as the Accept / Reject chip backgrounds on the decision divider, so set a `bg` and modifiers on them, never an `fg`** — `prompt_chip_style` pins the chip's foreground from `theme.normal`, so a foreground here reaches the full-row fill but is dropped on the chip. That is a convention, enforced at the render site rather than by the file format: both fields stay mirrored in `ThemeFile` and fully user-authorable, and the chip takes only the wash's `bg` + modifiers so an authored `fg` can't produce an unreadable label. **Do not "harden" this by removing them from `ThemeFile`.** `ThemeFile` is also the *export* format behind "Create custom theme", and `themes::util::blend` is a no-op on non-RGB colors — so dropping the section makes exporting `256 Dark` / `256 Light` / `Monochrome Dark` collapse both washes onto `surface`, taking the focused-hunk fill and both chips with it, and leaves indexed-palette user themes with no way to author a fill at all. Rethemeing through the `diff_add` / `diff_delete` palette entries (which ripple into all six derived slots) works only on an RGB palette; the three built-ins above hand-pick the wash precisely because it doesn't. Pinned by `theme_file::tests::focused_diff_washes_are_user_authorable`, `theme_file::tests::exporting_an_indexed_builtin_preserves_its_hand_picked_washes`, and `tests/diff_view.rs::{prompt_chips_wear_the_wash_of_the_side_they_name, prompt_chip_style_ignores_a_wash_foreground}`.
