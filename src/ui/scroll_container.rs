...
pub fn centered_rect_for_content(content: ContentSize, area: Rect) -> Rect {
    // Border is 2 cells; we add 1 cell of horizontal padding on each
    // side so text doesn't kiss the frame.
    let modal_width = (content.width).saturating_add(4).min(area.width);
    // Border is 2 cells (top + bottom); inside we want pinned_top +
    // height (scrolling region) + pinned_bottom rows.
