...
Use GitHub's private vulnerability reporting: [**Report a vulnerability**](https://github.com/mijowi/edamame/security/advisories/new).

Please include what you'd put in a bug report — version, OS, terminal — plus a proof-of-concept document if the issue is triggered by file content.
...
edamame is built on the assumption that **the documents it opens are untrusted** — you might open a `.md` file from a repository, a download, or an AI agent. Anything that lets document content escape that boundary is in scope, in particular:
